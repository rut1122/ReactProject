...
import { useParams, useNavigate } from "react-router-dom";
import TaskCard from "./TaskCard";
import { Typography, Box, Grid, Button, Paper, Stack, Container } from '@mui/material';
import ArrowForwardIcon from '@mui/icons-material/ArrowForward';
import AddIcon from '@mui/icons-material/Add';
...
const TasksList = () => {
  ...
  if (!project) return <Typography sx={{ textAlign: 'center', mt: 5 }}>טוען פרויקט...</Typography>;

  return (
    <Container maxWidth={false} disableGutters sx={{ minHeight: '100vh', bgcolor: '#f5f5f5', pb: 5 }}>
      {/* Header ברור ורחב */}
      <Box sx={{ width: '100%', bgcolor: 'white', py: 2, px: 4, mb: 4, boxShadow: '0 2px 10px rgba(0,0,0,0.05)', direction: 'rtl' }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          ...
          <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#b85e5e' }}>
            {project.name}
          </Typography>
          <Button 
            variant="contained" 
            ...
            onClick={() => navigate(`/Projects/${projectId}/AddTask`)} 
            sx={{ bgcolor: '#b85e5e', '&:hover': { bgcolor: '#a34f4f' }, borderRadius: '20px' }}
          >
            משימה חדשה
          </Button>
        </Stack>
          ...
          {statuses.map((status) => (
            <Grid item xs={12} sm={6} md={3} key={status.id}>
              <Paper elevation={0} sx={{ 
                ...
                <Stack spacing={2}>
                  {tasks.filter(t => t.status === status.id).map(t => (
                    <TaskCard key={t.id} task={t} />
                  ))}
                  {tasks.filter(t => t.status === status.id).length === 0 && (
                    <Typography variant="body2" sx={{ color: '#999', textAlign: 'center', mt: 5 }}>
                      אין משימות
                    </Typography>
                  )}
                </Stack>
